            ...
            bindevent: function () {
                $.validator.config({
                    rules: {
                        ...
                        morethan: function (element, params) {
                            var origin = element.value.toString();
                            if (!origin.match(/^\d+$/)) {
                                return '必须是数字';
                            }
                            var value = $("#" + params[0]).val();
                            if (origin != '0' && value != '0' && parseInt(origin) < parseInt(value)) {
                                return '必须大于' + value + "或为0";
                            }
                            return true;
                        },
                        lessthan: function (element, params) {
                            var origin = element.value.toString();
                            if (!origin.match(/^\d+$/)) {
                                return '必须是数字';
                            }
                ...
                require(['cityselect', 'citydata'], function (undefined, cityData) {
                    var mcs = $('#mcs').citySelect({
                        ...
                        onCallerAfter: function (target, values) {  //选择后回调
                            $("#c-limitarea").val(typeof values !== 'undefined' ? values.id.join(",") : '');
                        }
                    });
